...
import { PRODUCT_UNITS, type HouseholdProduct, type InventoryStatus, type ShoppingProfile } from "./types";
...
export function HouseholdDashboard() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: profile, error: profileError } = await supabase
      .from("user_profiles")
      .select("household_id,city,display_name,shopping_profile")
      .single();
    if (profileError || !profile?.household_id) {
      setMessage("Não foi possível carregar seu perfil."); setLoading(false); return;
    }
    setHouseholdId(profile.household_id); setCity(profile.city ?? ""); setDisplayName(profile.display_name ?? "");
  ...
  async function saveHouse(event: FormEvent) {
    ...
    const cleanCity = city.trim(); const cleanName = houseName.trim() || "Minha Casa";
    const { error: houseError } = await supabase.from("households").update({ name: cleanName, city: cleanCity || null }).eq("id", householdId);
  ...
  async function addProduct(event: FormEvent<HTMLFormElement>) {
    ...
    const brand = String(form.get("brand") ?? "").trim(); const unit = String(form.get("unit") ?? "unit");
    const packageQuantity = Number(form.get("packageQuantity") ?? 1); const stock = Number(form.get("stock") ?? 0);
    if (!name || packageQuantity <= 0 || stock < 0) { setMessage("Revise nome e quantidades do produto."); return; }
    const { data: product, error } = await supabase.from("products").insert({ household_id: householdId, name, brand: brand || null, package_quantity: packageQuantity, package_unit: unit }).select("id").single();
    if (error || !product) { setMessage("Não foi possível cadastrar o produto."); return; }
    const { error: inventoryError } = await supabase.from("inventory_items").insert({ household_id: householdId, product_id: product.id, registered_quantity: stock, estimated_quantity: stock, unit });
    if (inventoryError) { await supabase.from("products").delete().eq("id", product.id); setMessage("Não foi possível criar o estoque do produto."); return; }
    event.currentTarget.reset(); setMessage("Produto adicionado à Minha Casa."); await load();
  }
  ...
  async function changeStock(product: HouseholdProduct, delta: number) {
    ...
    const next = Math.max(0, Number(inventory.registered_quantity) + delta);
    ...
    setMessage(error ? "Não foi possível alterar o estoque." : "Estoque atualizado."); if (!error) await load();
  }
  ...
  async function setStatus(product: HouseholdProduct, status: InventoryStatus) {
    ...
    setMessage(error ? "Não foi possível alterar o status." : "Status atualizado."); if (!error) await load();
  }
  ...
  async function removeProduct(product: HouseholdProduct) {
    ...
    setMessage(error ? "Não foi possível remover o produto." : "Produto removido."); if (!error) await load();
  }
  ...
  return <main className="app-shell">
    <header className="topbar"><div><span className="badge">Sprint 1</span><h1>Minha Casa</h1><p>Produtos e estoque que realmente existem na sua casa.</p></div><button className="button secondary" onClick={signOut}>Sair</button></header>
